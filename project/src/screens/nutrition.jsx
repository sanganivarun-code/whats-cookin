...
const Nutrition = ({ go }) => {
  ...
  const weekly = dayTotals.reduce((a, d) => ({
    kcal: a.kcal + d.kcal,
    p: a.p + d.p, c: a.c + d.c, fat: a.fat + d.fat,
    sugar: a.sugar + d.sugar, fiber: a.fiber + d.fiber,
  }), { kcal: 0, p: 0, c: 0, fat: 0, sugar: 0, fiber: 0 });
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 24, alignItems: "start" }}>
        ...
        <div className="card">
          <div className="between mb-4">
            ...
            <div className="legend">
              <div className="legend-item"><span className="legend-swatch" style={{ background: "var(--paprika)" }}></span> Protein</div>
              <div className="legend-item"><span className="legend-swatch" style={{ background: "var(--saffron)" }}></span> Carbs</div>
              <div className="legend-item"><span className="legend-swatch" style={{ background: "var(--olive)" }}></span> Fat</div>
            </div>
          </div>
          ...
          <div style={{ position: "relative", paddingTop: 16 }}>
            ...
            <div style={{
              position: "absolute",
              top: `${16 + 180 - (1900 / maxKcal * 180)}px`,
              left: 0, right: 0,
              height: 1,
              ...
              <span style={{
                ...
                color: "var(--paprika)", letterSpacing: "0.08em",
            ...
            <div className="chart-bars">
              {DAYS.map((day, i) => {
                const d = dayTotals[i];
                const heightPx = (d.kcal / maxKcal) * 180;
                const pKcal = d.p * 4;
                const cKcal = d.c * 4;
                const fKcal = d.fat * 9;
                const denom = pKcal + cKcal + fKcal;
                return (
                  <div className={`chart-bar ${i === TODAY_INDEX ? "today" : ""}`} key={day.name}>
                    <div className="stack" style={{ height: heightPx }}>
                      <span style={{ background: "var(--olive)", flex: `${fKcal / denom} 0 0` }}></span>
                      <span style={{ background: "var(--saffron)", flex: `${cKcal / denom} 0 0` }}></span>
                      <span style={{ background: "var(--paprika)", flex: `${pKcal / denom} 0 0` }}></span>
                    </div>
        ...
        <div className="card macro-card">
          ...
          <div className="bar-row" style={{ marginBottom: 14 }}>
            ...
            <div className="macro-bar"><span className="fill" style={{ width: `${(avg.p * 4 / avg.kcal) * 100}%`, background: "var(--paprika)" }}></span></div>
            <span className="num">{avg.p}g · {Math.round((avg.p * 4 / avg.kcal) * 100)}%</span>
          </div>
          <div className="bar-row" style={{ marginBottom: 14 }}>
            <span className="lbl">Carbs</span>
            <div className="macro-bar"><span className="fill" style={{ width: `${(avg.c * 4 / avg.kcal) * 100}%`, background: "var(--saffron)" }}></span></div>
            <span className="num">{avg.c}g · {Math.round((avg.c * 4 / avg.kcal) * 100)}%</span>
          </div>
          <div className="bar-row" style={{ marginBottom: 14 }}>
            <span className="lbl">Fat</span>
            <div className="macro-bar"><span className="fill" style={{ width: `${(avg.fat * 9 / avg.kcal) * 100}%`, background: "var(--olive)" }}></span></div>
            <span className="num">{avg.fat}g · {Math.round((avg.fat * 9 / avg.kcal) * 100)}%</span>
          </div>
          ...
          <div style={{ borderTop: "1px solid var(--hairline)", paddingTop: 16, marginTop: 8 }}>
            <div className="bar-row" style={{ marginBottom: 10 }}>
              ...
              <div className="macro-bar"><span className="fill" style={{ width: `${Math.min(100, (avg.sugar / 25) * 100)}%`, background: "var(--paprika)" }}></span></div>
              <span className="num">{avg.sugar}g / 25g</span>
            </div>
            <div className="bar-row">
              <span className="lbl">Fibre</span>
              <div className="macro-bar"><span className="fill" style={{ width: `${Math.min(100, (avg.fiber / 30) * 100)}%`, background: "var(--olive)" }}></span></div>
              ...
              <div key={row.name} className="row gap-3" style={{ padding: "10px 0", borderBottom: "1px dashed var(--hairline)" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div className="macro-bar" style={{ height: 4 }}>
                    <span className="fill" style={{ width: `${(row.g / max) * 100}%`, background: "var(--paprika)" }}></span>
                  </div>
                </div>
              </div>
            );
