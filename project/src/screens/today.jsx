// Today — a stripped-down view focused on just today's four meals.
...
const Today = ({ go }) => {
  ...
  const [cooked, setCooked] = React.useState({});
  ...
  // Build the resolved meal list (respecting overrides)
  const meals = TODAY_MEAL_SLOTS.map((slot) => {
    if (slot.key === "lunch" && planDay.leftover) {
      const m = MEALS[planDay.leftover];
      return { ...slot, meal: m, leftover: true, key: slot.key };
    }
    const override = getOverride(dayIndex, slot.key);
    if (override?.kind === "removed") return { ...slot, removed: true };
    if (override?.kind === "eating-out") return { ...slot, eatingOut: true };
    if (override?.kind === "self-cook" || override?.kind === "custom") {
      const m = override.kind === "custom" ? MEALS[override.mealId] : null;
      return { ...slot, selfCook: true, name: override.name, meal: m };
    }
    return { ...slot, meal: MEALS[planDay[slot.key]] };
  }).filter((m) => !m.removed);

  // Totals from cookable meals only
  ...
  return (
    <div className="page page-narrow today-page">
      <div className="today-header">
        <div className="eyebrow mb-2">{greeting}, Aanya · {day.name}, {day.date}</div>
        ...
        <p className="lead mt-4">
          ...
          {totalCookTime > 0 ? <> roughly <strong style={{ color: "var(--ink)" }}>{totalCookTime} minutes</strong> of active cooking.</> : " nothing on the stove today."}
        </p>
      </div>
      ...
      <div className="today-progress">
        <div className="row gap-3" style={{ alignItems: "center" }}>
          ...
          <span className="muted" style={{ fontSize: 13 }}>
            {cookedCount === 0 ? "Nothing cooked yet — let's begin." :
             cookedCount === meals.length ? "Done for the day. Well fed." :
             "On your way."}
          </span>
        </div>
        <div className="today-progress-bar">
          <span className="fill" style={{ width: `${(cookedCount / meals.length) * 100}%` }}></span>
        </div>
      </div>
          ...
          return (
            <div key={id} className={`today-meal ${isCooked ? "cooked" : ""} ${m.leftover ? "leftover" : ""}`}>
              <div className="today-meal-head">
                ...
                <button className={`heart-static ${isFav ? "on" : ""}`}
              ...
              <div className="row gap-4 mt-2" style={{ alignItems: "center" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div className="row gap-3 mt-2" style={{ flexWrap: "wrap" }}>
                    ...
                    <span className="today-stat"><span className="dot" style={{ background: "var(--paprika)" }}></span> {meal.p}g protein</span>
              ...
              <div className="today-meal-actions">
                <button className="btn btn-ghost btn-sm" onClick={() => go("recipe")}>
      ...
      <div className="today-footer">
        <div>
          ...
          <div className="muted mt-2" style={{ fontSize: 13 }}>
            Target: 1,900 kcal · 110g protein
          </div>
        </div>
