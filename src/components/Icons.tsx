...
const GLYPH_MAP: Record<GlyphKind, () => JSX.Element> = {
  Bowl, Roti, Glass, Leaf, Stack, Dome, Pot, Rice, Cube,
}

const TONE_COLOR: Record<ColorTone, string> = {
  paprika: 'var(--paprika)',
  saffron: 'var(--saffron)',
  olive:   'var(--olive)',
  ink:     'var(--ink)',
}
const TONE_BG: Record<ColorTone, string> = {
...
export function FoodGlyph({ kind, tone = 'paprika', size = 'md' }: FoodGlyphProps) {
  const Comp = GLYPH_MAP[kind] ?? Bowl
  return (
    <div
      className={`glyph${size === 'lg' ? ' lg' : size === 'sm' ? ' sm' : ''}`}
...
export const Icon = {
  ...
  Leaf: ({ size = 14 }: { size?: number }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round">
      <path d="M5 19c0-10 6-14 14-14 0 8-4 14-14 14z" />
      <path d="M5 19L13 11" />
    </svg>
  ),
  Heart: ({ size = 14, filled = false }: { size?: number; filled?: boolean }) => (
